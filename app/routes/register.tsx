      ...
      <div className="w-full max-w-md">
        <div className="bg-white rounded-3xl shadow-sm border border-gray-200 p-8">
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
            {[
              { id: "fullName", label: "Full Name", type: "text", placeholder: "Alex Johnson" },
              ...
              <div key={id}>
                ...
                {errors[id as keyof RegisterForm] && (
                  <p className="text-xs text-red-500 mt-1.5">
                    {errors[id as keyof RegisterForm]?.message}
                  </p>
                )}
              </div>
            ))}
          ...
          <p className="text-center text-sm text-gray-500 mt-6">
            ...
            <Link to="/login" className="font-bold text-gray-900 hover:text-yellow-600 transition-colors">Sign in</Link>
          </p>
        </div>
      </div>
    </div>
