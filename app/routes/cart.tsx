...
function buildWhatsAppMessage(items: ReturnType<typeof useCartStore.getState>["items"]) {
  const lines = items.map(
    (item) =>
      `• *${item.name}*${item.size ? ` (Size: ${item.size})` : ""}${item.color ? `, Color: ${item.color}` : ""} × ${item.quantity}`
  );
  const msg = [
    "Hi! I'd like to place an order on *yellowbirds* 🐦",
    "",
    ...lines,
    "",
    "Please confirm pricing, availability, and delivery details. Thank you!",
  ].join("\n");
          ...
          {items.map((item) => (
            <div key={item.variantId} className="flex gap-4 p-4 rounded-2xl border border-gray-200 bg-white hover:border-yellow-300 transition-colors">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-sm font-semibold text-yellow-600 mt-1">Price negotiated on WhatsApp</p>
          ...
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div className="space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-gray-500">{items.reduce((s, i) => s + i.quantity, 0)} item{items.reduce((s, i) => s + i.quantity, 0) !== 1 ? "s" : ""} in cart</span>
              </div>
              <div className="flex justify-between">
              ...
              <div className="flex justify-between">
                ...
                <span className="font-semibold text-yellow-600">Negotiated on WhatsApp</span>
              </div>
            </div>
            ...
            <a
              ...
              className="mt-5 flex items-center justify-center gap-2.5 w-full py-4 rounded-xl font-extrabold text-base text-white transition-colors"
            ...
            <p className="text-xs text-gray-400 text-center mt-3">
              Opens WhatsApp with your full order. Our team will confirm and arrange delivery.
            </p>
          </div>
