...
export default function CartPage() {
  ...
  async function handlePlaceOrder() {
    ...
    try {
      ...
      window.open(buildWhatsAppMessage(selectedItems, order.id), "_blank", "noopener,noreferrer");

      // Only remove the items that were ordered
      selectedItems.forEach((i) => removeItem(i.variantId));
      setSelectedIds(new Set());
        ...
        <div className="lg:col-span-2 space-y-3">
          ...
          <div className="flex items-center gap-3 px-1 pb-1">
            <input
              ...
              onChange={toggleAll}
              className="h-4 w-4 rounded border-gray-300 text-yellow-400 accent-yellow-400 cursor-pointer"
            />
            <label htmlFor="select-all" className="text-sm font-semibold text-gray-600 cursor-pointer select-none">
              ...
              >
                ...
                <div className="flex items-center shrink-0 pt-1">
                  <input
                    ...
                    className="h-4 w-4 rounded border-gray-300 accent-yellow-400 cursor-pointer"
                  />
                </div>
        ...
        <div className="space-y-4">
          <div className="bg-white border border-gray-200 rounded-2xl p-6">
            ...
            <div className="space-y-2 text-sm mb-5">
              ...
              <div className="flex justify-between">
                <span className="text-gray-500">Remaining in cart</span>
                <span className="text-gray-400">
                  {items.filter((i) => !selectedIds.has(i.variantId)).reduce((s, i) => s + i.quantity, 0)} item{items.filter((i) => !selectedIds.has(i.variantId)).reduce((s, i) => s + i.quantity, 0) !== 1 ? "s" : ""}
                </span>
              </div>
              <div className="flex justify-between">
            ...
            <button
              ...
              className="flex items-center justify-center gap-2.5 w-full py-4 rounded-xl font-extrabold text-base text-white transition-colors disabled:opacity-60 disabled:cursor-not-allowed"
              style={{ backgroundColor: "#25D366" }}
              onMouseOver={e => { if (!placing && selectedItems.length > 0) (e.currentTarget as HTMLButtonElement).style.backgroundColor = "#20b858"; }}
            ...
            <p className="text-xs text-gray-400 text-center mt-3">
              Only selected items will be ordered. Unselected items stay in your cart.
            </p>
          </div>
